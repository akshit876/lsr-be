...
class AlarmServiceTester {
  ...
  setupEventListeners() {
    ...
    this.alarmService.io.on('safety_violation', (data) => {
      ...
      logger.info(`🚨 Safety Violation Event #${this.eventCounts.safety_violation}:`, {
    ...
    this.alarmService.io.on('safety_violation_cleared', (data) => {
      ...
      logger.info(`✅ Safety Violation Cleared Event #${this.eventCounts.safety_violation_cleared}:`, {
        violation: data.violation,
        value: data.value,
        timestamp: data.timestamp
      });
  ...
  printResults() {
    logger.info('\n📊 Test Results:');
    logger.info('================');
    logger.info(`Safety Violation Events: ${this.eventCounts.safety_violation}`);
    logger.info(`Safety Violation Cleared Events: ${this.eventCounts.safety_violation_cleared}`);
    logger.info(`Total Events: ${this.eventCounts.safety_violation + this.eventCounts.safety_violation_cleared}`);
    
    if (this.lastEventTime.safety_violation) {
      logger.info(`Last Safety Violation: ${this.lastEventTime.safety_violation}`);
    }
    if (this.lastEventTime.safety_violation_cleared) {
      logger.info(`Last Safety Violation Cleared: ${this.lastEventTime.safety_violation_cleared}`);
    }

    // Analyze the results
    const totalEvents = this.eventCounts.safety_violation + this.eventCounts.safety_violation_cleared;
    const expectedMaxEvents = 10; // Should be much less than 300 (30 seconds * 10 events/second)
    
    if (totalEvents > expectedMaxEvents) {
      logger.error(`❌ FAIL: Too many events (${totalEvents}). Expected < ${expectedMaxEvents}`);
      logger.error('   This indicates the service is still emitting events continuously.');
    } else {
      logger.success(`✅ PASS: Event count (${totalEvents}) is within expected range`);
      logger.success('   The service is properly tracking state and not emitting duplicate events.');
    }
  }
