...
io.on("connection", (socket) => {
  ...
  // New event for manual entry workflow
  socket.on("manual-entry-submit", async (requestData) => {
    try {
      const { manualData, modelNumber } = requestData;

      if (!manualData || manualData.trim() === "") {
        socket.emit("manual-entry-error", {
      ...
      logger.info(
        `📝 Manual entry received from client ${socket.id}: ${manualData}`
      );

      // Import scanner controller dynamically to avoid circular imports
      const { scannerController } = await import("./services/scanCycles.js");

      // Acknowledge receipt
      socket.emit("manual-entry-received", {
        manualData: manualData,
        timestamp: new Date().toISOString(),
        message: "Manual data received, starting cycle...",
      });

      // Start manual entry cycle
      await scannerController.runManualEntryCycle(io, manualData);
    } catch (error) {
      logger.error(
        `Error processing manual entry for client ${socket.id}:`,
        error
      );
      socket.emit("manual-entry-error", {
  ...
  // Event to check manual entry system status
  socket.on("check-manual-entry-status", async () => {
    try {
      const { scannerController } = await import("./services/scanCycles.js");

      socket.emit("manual-entry-status", {
        isReady: !scannerController.isRunning,
        cycleCount: scannerController.cycleCount,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      logger.error(
        `Error checking manual entry status for client ${socket.id}:`,
        error
      );
      socket.emit("error", {
        message: "Failed to check manual entry status",
        details: error.message,
      });
    }
  });

  // Event to reset manual entry cycle count
  socket.on("reset-manual-entry-cycle", async () => {
    try {
      const { scannerController } = await import("./services/scanCycles.js");

      scannerController.resetCycleCount();
      logger.info(`Manual entry cycle count reset by client ${socket.id}`);

      socket.emit("manual-entry-cycle-reset", {
        message: "Cycle count reset successfully",
        newCycleCount: scannerController.cycleCount,
        timestamp: new Date().toISOString(),
      });
    } catch (error) {
      logger.error(
        `Error resetting manual entry cycle for client ${socket.id}:`,
        error
      );
      socket.emit("error", {
        message: "Failed to reset manual entry cycle",
        details: error.message,
      });
    }
  });
