...
class ScannerController {
  ...
  // New method for manual entry workflow (new machine)
  async runManualEntryCycle(io = null, manualData) {
    this.io = io;
    this.isRunning = true;

    logger.info("🚀 Starting manual entry cycle workflow...");
    logger.info(`📝 Manual data received: ${manualData}`);

    try {
      // Step 1: Validate manual data
      if (!manualData || manualData.trim() === "") {
        throw new Error("Manual data is required for this workflow");
      }

      // Step 2: Wait for cycle start signal
      logger.info("⏳ Waiting for cycle start signal (1410.0)...");
      const resetResult = await this.checkResetOrBit(1410, 0, 1);
      if (resetResult === true) {
        logger.info("Reset detected, restarting cycle");
        return;
      ...
      // Step 3: Generate barcode using manual data
      const barcodeData = await this.generateManualEntryBarcode(manualData);
      if (!barcodeData) {
        logger.error("❌ Failed to generate barcode from manual data");
        return;
      }

      // Step 4: Signal transfer completion
      logger.info("✍️ Writing bit 1414.15(F) to signal file transfer");
      await writeBit(1414, 15, 1);

      // Step 5: Wait for cycle end signal
      logger.info("⏳ Waiting for cycle end signal (1415.7)...");
      const cycleEndResult = await this.checkResetOrBit(1415, 7, 1);
      if (cycleEndResult === true) {
        logger.warn(
          "⚠️ Reset detected while waiting for cycle end, restarting"
        );
        return;
      }

      // Step 6: Complete cycle
      await this.completeManualEntryCycle(barcodeData, true);
      logger.success("✅ Manual entry cycle completed successfully");
    } catch (error) {
      logger.error("❌ Error in manual entry cycle:", error);
      await this.completeManualEntryCycle(null, false, error.message);
      throw error;
    }
  }

  // Generate barcode for manual entry workflow
  async generateManualEntryBarcode(manualData) {
    try {
      logger.info("🏷️ Starting manual entry barcode generation...");
      logger.info(`📝 Using manual data: ${manualData}`);

      // Get current model and generate serial number
      const modelNumber = await this.getCurrentModelNumber();
      const currentId = await this.getCurrentDayId();

      // Create barcode text using manual data
      const barcodeText = manualData.trim();
      const serialString = `${modelNumber || "MANUAL"}-${currentId}`;

      logger.info(`✅ Barcode generated: ${barcodeText}`);
      logger.info(`🔢 Serial Number: ${serialString}`);

      // Write barcode to files
      logger.info("📁 Writing barcode data to files...");
      await Promise.all([
        this.writeToFile(CODE_FILE_PATH, barcodeText, "Manual barcode data"),
        this.writeToFile(TEXT_FILE_PATH, barcodeText, "Manual barcode text"),
      ]);
      logger.info("✅ Files written successfully");

      // Emit marking data to UI
      if (this.io) {
        logger.info("📡 Emitting manual marking data to UI...");
        this.io.emit("marking_data", {
          timestamp: new Date(),
          data: barcodeText,
          type: "manual_entry",
      ...
      if (isVerified) {
        // Save to MongoDB
        logger.info("💾 Saving manual entry data to MongoDB...");
        await this.saveToMongoDB({
          io: this.io,
          serialNumber: serialString,
          markingData: barcodeText,
      ...
      logger.error("❌ Error in manual entry barcode generation:", error);
      throw error;
    }
  }

  // Complete manual entry cycle
  async completeManualEntryCycle(barcodeData, success, errorMessage = null) {
    try {
      this.cycleCount++;

      if (success && barcodeData) {
        logger.section(`✅ Completed Manual Entry Cycle ${this.cycleCount}`);
        ...
        logger.section(`❌ Failed Manual Entry Cycle ${this.cycleCount}`);

        // Save failed cycle data
        await this.saveToMongoDB({
          io: this.io,
          serialNumber: errorMessage ? "ERROR" : "N/A",
          markingData: errorMessage || "CYCLE_FAILED",
          scannerData: "MANUAL_ENTRY",
          result: "NG",
          grading: "MANUAL",
          isUpdate: false,
      ...
      // Trigger UI refresh
      if (this.io) {
        logger.info("📡 Broadcasting manual entry cycle completion to UI...");
        await mongoDbService.broadcastDataToAllClients(
          this.io,
          "main-data",
          "records"
        ...
        this.io.emit("manual-cycle-completed", {
          cycleNumber: this.cycleCount,
          timestamp: new Date().toISOString(),
          success: success,
          result: success ? "OK" : "NG",
          data: barcodeData ? barcodeData.text : null,
          error: errorMessage,
        });
      }

      logger.info(`🎯 Manual entry cycle count: ${this.cycleCount}`);
    } catch (error) {
      logger.error("❌ Error completing manual entry cycle:", error);
      throw error;
    }
  }
}
