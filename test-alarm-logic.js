...
function runTests() {
  ...
  logger.info(
    `Safety Violation Cleared Events: ${counts.safety_violation_cleared}`
  );
  logger.info(`Status Update Events: ${counts.status_updates}`);
  logger.info(
    `Total Events: ${counts.safety_violation + counts.safety_violation_cleared + counts.status_updates}`
  );
  ...
  if (actualEvents <= expectedEvents) {
    logger.success(
      `✅ PASS: Event count (${actualEvents}) is within expected range (≤${expectedEvents})`
    );
    logger.success(
      "   The alarm service properly tracks state and prevents duplicate events."
    );
  } else {
    logger.error(
      `❌ FAIL: Event count (${actualEvents}) exceeds expected range (≤${expectedEvents})`
    );
